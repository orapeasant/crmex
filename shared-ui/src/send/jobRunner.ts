...
import { buildQueue, type BuiltQueue } from './queueBuilder.js';

export interface JobRunnerDeps {
  supabase: SupabaseClient;
  userId: string;
  checkRegistered(jids: string[]): Promise<Record<string, boolean>>;
  /** Send the queue and resolve once the whole batch has finished. */
  sendBatch(orgId: string, batchId: string, queue: BuiltQueue): Promise<void>;
}

export type JobOutcome = 'not-claimed' | 'done' | 'failed';
...
export async function runSendJob(deps: JobRunnerDeps, job: SendJobRow): Promise<JobOutcome> {
  ...
  try {
    ...
    // Suppression is re-read now, not trusted from queue time: a client may have opted out since.
    const { data, error } = await supabase.from('clients').select('id, phone_e164, suppressed_at').eq('org_id', claimed.org_id);
    if (error) throw error;
    const clients = (data ?? []) as { id: string; phone_e164: string | null; suppressed_at: string | null }[];
    ...
    const jids = Array.from(new Set(recipients.map((r) => r.jid)));
    ...
    await deps.sendBatch(claimed.org_id, claimed.id, queue);
