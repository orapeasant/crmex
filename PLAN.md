...
16. **Campaigns (§18, after approval)** — `send_jobs` schedule/pacing columns, `POST /images/upload` for pasted images, the schedule-and-pace wizard steps, a time-aware `jobRunner`, and the campaign list in both shells.
17. **Occasion rules (§19, after approval, needs §16)** — `occasion_rules` / `client_dates` / `occasion_occurrences`, the hourly scanner with its idempotent insert and stagger, and the rule UI under Settings.
...
- **Scheduled bulk send / campaigns** (`crmex.md` §18, designed 2026-09-18, under review): resolve C-D1–C-D5 in §18.8 (notably the default pacing interval and `pacing.min_interval_ms` floor, which overlap the pacing-defaults item above), then add its build steps (§18.10) to §4 and remove "campaign scheduling" from the out-of-scope list in §3. Adds `clients.status` (`active`/`inactive`/`archived`, §18.3.1 — a CRM lifecycle flag kept separate from the `suppressed_at` opt-out) and extends `send_jobs` with `scheduled_at` / `interval_ms` / `jitter_pct` / `expires_at`; the sender's phone remains the only sender, positioned as an always-on gateway, with FCM wake-up deferred to §18.9.
